...
const readAllSMS = async () => {
  ...
  const filter = { box: "" }; // inbox + sent + draft

  SmsAndroid.list(
    JSON.stringify(filter),
    fail => console.log("❌ SMS Fetch Failed:", fail),
    (count, smsList) => {
      console.log("📨 Total SMS Found:", count);
...
const listenForUnreadFinanceSMS = async () => {
  ...
  SmsListener.addListener(async (message) => {
    // Only process SMS from 8810 that are unread
    if (message.originatingAddress === '8810' && message.read === 0) {
      const data = parseFinanceSMS(message.body);

      if (data && data.amount) {
        console.log('💰 New Unread Finance SMS:', data);

        // // Send to backend API
        // try {
        //   await axios.post(`${API_BASE_URL}/api/save-finance`, data);
        //   console.log('✅ Finance data saved to backend');
        // } catch (err) {
        //   console.log('❌ Failed to save finance data:', err.message);
        // }

        // Show notification
        Alert.alert(
          'New Transaction',
          `${data.type?.toUpperCase()} of PKR ${data.amount} from ${data.source}`
        );
      }
    }
  });
